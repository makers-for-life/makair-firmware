...
// Linearisation has been made experimentaly.
uint16_t PressureValve::openLinear(uint16_t p_command) {

    // The p_command is in [ 0 ; 125 ], but the valve is only effective in [30; 100]. So lets make
    // it between 30 and 100. A x10 multiplier is used here for better precision with integer
    // calculations. The value of intermediateValue will consequently be in [300 ; 1000]
    uint32_t intermediateValue = (uint32_t)((p_command * 70u / 125u + 30u) * 10u);

    /* An order 3 polynom is used to correct the non linearity of the valve. To find this polynom,
       ...
       Then we manually found for each openning value, the openning value that should have been used to
       reach the targetFlow :
       openningValue[0;125], flow (mL/min), targetFlow (mL/min), correctedOpenningValue[0;125]
       0, 48137,48137, 30
       10, 47780, 44296, 55.2
       ...
       125, 127, 0, 100

       Then when made an order 3 regression between correctedOpenningValue and openningValue. This is the relation below :
    */
    command = (uint16_t)(
        (76u * intermediateValue / 10u - (985u * intermediateValue * intermediateValue / 100000u)
         + (((44u * intermediateValue * intermediateValue) / 1000u) * intermediateValue) / 10000u
         - 1140u)
        / 10u);
